...
Бот работает в связке VK-Telegram. Состоит из 2 частей. Первая часть это бот для VK, добавляется в любой чат, может читать сообщения. Вторая часть это бот для Telegram, занимается тем, что собирает сообщения из чата VK и пересылает в Telegram.  

![alt text](https://github.com/traster100/workflow/blob/master/Portfolio/imgs/24.png "Бот для Telegram")
